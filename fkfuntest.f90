...
psi2(dimz+1) = 0.0 ! wall, no charge
psi2(0) = psi2(1)  ! wall, no charge

fbound = 0.0

do i=1,n
   xpos(iz) = expmupos*(xh(iz)**vsalt)*exp(-psi2(iz)*zpos) ! ion plus volume fraction
   xneg(iz) = expmuneg*(xh(iz)**vsalt)*exp(-psi2(iz)*zneg) ! ion neg volume fraction
   xHplus(iz) = expmuHplus*(xh(iz))*exp(-psi2(iz))         ! H+ volume fraction
   xOHmin(iz) = expmuOHmin*(xh(iz))*exp(+psi2(iz))         ! OH-  volume fraction
enddo
...
do i = 1,ntot
xtotal(i) = 1.0 - xh(i) -xpos(iz)-xneg(iz)-xHplus(iz)-xOHmin(iz)
enddo
! stoichoimetry
...
do i=1,n
 qtot(i) = (zpos*xpos(i)+zneg*xneg(i))/vsalt + avpolA(i)*zpolA/vpol*(1.0-fdisANC(i)-fdisAas(i)-fdisANa(i))&
+ avpolB(i)*zpolB/vpol*(1.0-fdisBNC(i)-fdisBas(i)-fdisBCl(i)) + xHplus(i)-xOHmin(i)
enddo

do i=1,n
  f(i+n)= psi2(i+1) -2*psi2(i) + psi2(i-1) + qtot(i)*constq
  f(i+n)=f(i+ntot)/(-2.0)
enddo
